...
export default function LoginPage() {
  ...
  // State Tampilan Form: "login" | "register" | "forgot"
  ...
  // State Cloudflare Turnstile Token
  ...
  // 1. Callback Turnstile & Redirect Handler dari Google OAuth
  useEffect(() => {
    // Handler Callback Turnstile
    window.onTurnstileSuccess = (token) => {
      setTurnstileToken(token);
    };

    // Handler Pengecekan URL Params dari Google Callback (Jika user belum terdaftar)
    const status = searchParams.get("status");
    const googleEmail = searchParams.get("email");
    const googleName = searchParams.get("nama") || searchParams.get("name");
    const gId = searchParams.get("google_id");

    if (status === "pending_registration" || searchParams.get("redirect_to") === "/register") {
      setView("register");
      if (googleEmail) setEmail(googleEmail);
      if (googleName) setFullName(googleName);
      if (gId) setGoogleId(gId);
      
      setSuccessMessage("Akun Google belum terdaftar. Silakan lengkapi nama perusahaan Anda untuk menyelesaikan registrasi.");
    }
  }, [searchParams]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/auth/login", {
        ...
        body: JSON.stringify({ email, password, cf_turnstile_token: turnstileToken }),
      });
      ...
      if (result.status === "success" || result.access_token) {
        ...
        const tenantSlug = result.tenant || "dashboard"; 
        router.push(tenantSlug.startsWith("/") ? tenantSlug : `/${tenantSlug}`);
      }
    } catch (err) {
  ...
  // -------------------------------------------------------------
  // HANDLER LOGIN VIA GOOGLE OAUTH2
  // -------------------------------------------------------------
  const handleGoogleLogin = () => {
    const googleAuthUrl = `http://localhost:8000/api/auth/google/login?cf_turnstile_token=${encodeURIComponent(turnstileToken || "")}`;
    window.location.href = googleAuthUrl;
  };

  // -------------------------------------------------------------
  // HANDLER DAFTAR (REGISTER)
  // -------------------------------------------------------------
        ...
        >
          <div className="row g-0">
            ...
            <div className="col-lg-6 d-flex align-items-center bg-white">
              <div className="card-body p-4 p-sm-5 mx-md-3 w-100">
                ...
                {view === "login" && (
                  <>
                    ...
                    <form onSubmit={handleLogin}>
                      ...
                      {/* CLOUDFLARE TURNSTILE WIDGET (DIBUAT PRETTY & RESPONSIVE) */}
                      <div className="turnstile-wrapper mb-3 d-flex justify-content-center overflow-hidden">
                        <div 
                    ...
                    <button 
                      ...
                      className="btn btn-outline-secondary w-100 fw-semibold d-flex align-items-center justify-content-center gap-2"
                      ...
                      <div className="mb-3">
                        ...
                        <div className="position-relative d-flex align-items-center">
                          ...
                          <input
                            ...
                            value={email}
                            onChange={(e) => setEmail(e.target.value)}
                            readOnly={!!googleId} // Readonly jika diisi otomatis dari Google
                            required
                          />
                        </div>
                      </div>
